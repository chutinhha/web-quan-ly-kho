...
function drawDayEvent() {
    if (eventSource != null) {
        ...
        $("#evxDay" + formatJsonSimple(result.StartDate)).append('<tr><td width="100%" valign="top"><img id="imgCalled' + result.Id + '" style="display:none" border="0" align="absmiddle" title="Liên lạc Khách hàng" src="' + urlRoot + 'images/img_Calendar/Telephone-icon.png"><img id="imgApproved' + result.Id + '" style="display:none" border="0" align="absmiddle" title="Đã chấp nhận" src="' + urlRoot + 'images/img_Calendar/icon_approved.gif"><img id="imgRejected' + result.Id + '" style="display:none" border="0" align="absmiddle" title="Đã từ chối" src="' + urlRoot + 'images/img_Calendar/icon_rejected.gif"><a id="lnkViewId' + result.Id + '" href="javascript:;" onclick="viewEventDetail(' + result.Id + ')">' + formatDateTime(result.StartDate) + ' ' + result.Title + '</a></td></tr>');
        /* cai dat hien thi cac icon trang thai */
            ...
            if (result.Status == 2) {
                ...
                $("#imgCalled" + result.Id).attr("style", "");
            }
        });
    }
}
