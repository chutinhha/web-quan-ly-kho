...
FontFace = "'Tahoma','Garamond Condensed','Times New Roman',Times,serif";
...
function initTreeView() { if (self.TVinitd) return;
 if (self.checkFrames && (""+window.innerWidth != "0")) 
{ // not printing
...
function tree(code) { var ret = "";
 ...
 for (var i = 0; i < code.length; i++) { var c = code.charAt(i);
  ...
  if (c == '#') ret += img("book", NormalPageHint);
  if (c == 'x') ret += img("link", LinkedPageHint);
  if (c == 'b') ret += img("open", ClosedBookHint);
