...
function addUpload(maxUploads, descFieldName, fileFieldName) {
    ...
    if (currentUploads > 0)
        document.getElementById('addupload').childNodes[0].data = 'Chọn thêm File';
...
function removeFile(container, item) {
    ...
    if (currentUploads == 0)
        document.getElementById('addupload').childNodes[0].data = 'Đính kèm File';

}

function addUpload1(maxUploads, descFieldName, fileFieldName) {
    nameDesc = descFieldName;
    nameFile = fileFieldName;
    max = Number(maxUploads);
    ...
    if (currentUploads > 0)
        document.getElementById('addupload1').childNodes[0].data = 'Chọn thêm File';

    if (currentUploads == max) document.getElementById('addupload1').style.visibility = 'hidden';


    // First, clone the hidden attachment section
    var newFields = document.getElementById('attachment1').cloneNode(true);
    newFields.id = '';
    ...
    var insertHere = document.getElementById('attachmentmarker1');
    insertHere.parentNode.insertBefore(newFields, insertHere);
}

function removeFile1(container, item) {
    // get the ID number of the upload section to remove
    var tmp = item.getElementsByTagName('input')[0];
    var basefieldname = '';
    if (tmp.type == 'text') basefieldname = nameDesc; else basefieldname = nameFile;
    ...
    // Shift all INPUT field IDs and NAMEs down by one (for fields with a
    // higher ID than the one being removed)
    var x = document.getElementById('attachments1').getElementsByTagName('input');
    for (i = 0; i < x.length; i++) {
        if (x[i].type == 'text') basefieldname = nameDesc; else basefieldname = nameFile;
    ...
    // Run through all the DropCap divs (the number to the right of the attachment
    // section) and update that number...
    x = document.getElementById('attachments1').getElementsByTagName('div');
    for (i = 0; i < x.length; i++) {
        // Verify this is actually the "dropcap" div
        if (x[i].id.substring(0, String('dropcap').length) == 'dropcap') {
            ID = Number(x[i].id.substring(String('dropcap').length, x[i].id.length));
    ...
    container.removeChild(item);
    setScrollPos();
    document.getElementById('addupload1').style.visibility = 'visible';
    if (currentUploads == 0)
        document.getElementById('addupload1').childNodes[0].data = 'Đính kèm File';
