...
if (ToolBar_Supported)
{
	...
	//=====================================================================
	var DefToolbarBGImage = false;
	var DefMSMenuPaneBGImage = false;
	var DefICPMenuPaneBGImage = false;
	//=====================================================================
	...
	if ( !RTL ){
		FooterStr = "" +
			"<TABLE ID='idFooter1' STYLE='background-color:white;' cellSpacing='0' cellPadding='0' border='0'>" +
	...
	HTMLStr = "<DIV ID='idToolbar' STYLE=' HEIGHT: 90px;width:100%; <!--TOOLBAR_BACKGROUND--> '>" ;
	
	HTMLStr += 
	"<DIV ID='idRow1'        STYLE='position:relative;height:20px;'>";

	if (!RTL){
		HTMLStr += "<DIV ID='idICPBanner'   STYLE='position:absolute;top:0px;left:0px;height:60px;width:100px;overflow:hidden;vertical-align:top;'><!--BEG_ICP_BANNER--><!--END_ICP_BANNER--></DIV>";
		HTMLStr += "<DIV ID='idMSMenuCurve' STYLE='position:absolute;top:0px;left:100px;height:20px;width:18px;overflow:hidden;vertical-align:top;'><!--BEG_MSMENU_CURVE--><!--END_MSMENU_CURVE--></DIV>";
		HTMLStr += "<DIV ID='idMSMenuPane'  STYLE='text-align: left; position:absolute;top:0px;left:100px;height:20px;width:10px;float:right; <!--MSMENUPANE_BACKGROUND-->' NOWRAP><!--MS_MENU_TITLES--></DIV>";
		HTMLStr += "</DIV>";
	}else if(RTL){
		TBLStr  = "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR STYLE='height:20;vertical-align:middle'><!--ICP_MENU_TITLES--></TR></TABLE>";
		HTMLStr += "<DIV ID='idICPBanner'   STYLE='position:absolute;top:0;left:0;height:60;width:100;overflow:hidden;vertical-align:top;'><!--BEG_ICP_BANNER--><!--END_ICP_BANNER--></DIV>";
		HTMLStr += "<DIV ID='idMSMenuCurve' STYLE='position:absolute;top:0;left:100;height:20;width:18;overflow:hidden;vertical-align:top;'><!--BEG_MSMENU_CURVE--><!--END_MSMENU_CURVE--></DIV>";
		// RTL Correction: added: dir='ltr' (forcing LTR!), added: &nbsp; (prevents last menu problem)
		HTMLStr += "<DIV dir='ltr' lang='he' ID='idMSMenuPane'  STYLE='text-align: left; position:absolute;top:0;left:100;height:20;width:10;color:white;float:left; <!--MSMENUPANE_BACKGROUND-->' NOWRAP><!--MS_MENU_TITLES-->&nbsp;</DIV>";
		HTMLStr += "</DIV>";
	}
	if(!RTL){
		HTMLStr += "<DIV ID='idRow2' STYLE='position:relative;left:100px;height:50px;'>";
		...
		HTMLStr += "<DIV ID='idRow2' STYLE='position:relative;left:000;height:40px;'>" ; // RTL Correction: was left:100
	...
	if(!RTL){
		...
		// zIndex Correction
		HTMLStr += "<DIV ID='idICPMenuPane' STYLE='position:absolute;top:0px;left:0px;height:20px; <!--ICPMENUPANE_BACKGROUND-->' NOWRAP><!--ICP_MENU_TITLES--></DIV>";
		HTMLStr += "</DIV>";
		HTMLStr += "</DIV>";
	}else if (RTL){
		HTMLStr += "<DIV ID='idRow3' STYLE='position:relative;height:20px;width:100%'>";
		// RTL Correction: added: dir='rtl'
		HTMLStr += "<DIV dir='rtl' ID='idICPMenuPane' STYLE='position:absolute;top:0;left:0;height:20px;color:white; <!--ICPMENUPANE_BACKGROUND-->' NOWRAP><!--ICP_MENU_TITLES--></DIV>";
	...
	ToolbarBGColor	= "#0055a5";
	
	if (Strict_Compat)
	{
		MSFont  = "bold 10 Verdana";//"bold x-small Arial";
	...
	else
	{
		if( !RTL ){
			...
			MSFont  = "bold 10 Verdana";//"bold xx-small Verdana";
			...
			//MNP1 RTL revision
			MSFTFont = "bold 10 Verdana";//"11px Arial";
	...
	aDefICPColor[0]	= aCurICPColor[0] = "#E96538";	// bgcolor;E96538
...
function drawToolbar()
{
	...
	//MSMenuWidth     = Math.max(idMSMenuPane.offsetWidth, (500+112));
	MSMenuWidth     = Math.max(idMSMenuPane.offsetWidth, (100+112));//MSMenuWidth trong TH nho nhat = (100+112)
	
	ToolbarMinWidth = (100+18) + MSMenuWidth;
		
	if (!DefToolbarBGImage)		idToolbar.style.backgroundColor = ToolbarBGColor;
	if (!DefMSMenuPaneBGImage)	idMSMenuPane.style.backgroundColor  = aDefMSColor[0];
	if (!DefICPMenuPaneBGImage)	idICPMenuPane.style.backgroundColor = aDefICPColor[0];
...
function resizeToolbar()
{
	...
	w = Math.max(ToolbarMinWidth, document.body.clientWidth) - ToolbarMinWidth-1;
	if ( !RTL ){
		if (document.all("idMSMenuCurve"))
		{	
			idMSMenuCurve.style.left  = (100+w) + 'px';
			idMSMenuPane.style.left   = (100+w+18) + 'px';
		...
		idMSMenuCurve.style.left  = MSMenuWidth;	// RTL Correction: was (100+w);
		idMSMenuPane.style.left   = 0;			// RTL Correction: was (100+w+18);
...
function addICPMenu(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, Popup, MenuFlagBGColor)
{ 	
	if(RTL){
	...
	if (addICPMenu.arguments.length > 5)
		TargetStr = addICPMenu.arguments[5];
	else
		TargetStr = "_top";
	tempID = "ICP_" + MenuIDStr;
	
	if(!Popup){
		addMenu(tempID, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, true, MenuFlagBGColor); 
	}else	
		addMenuOpenPopup(tempID, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, true);
...
function addMSMenu(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, MenuFlagBGColor)
{	
	TargetStr = "_top";
	tempID = "MS_" + MenuIDStr;
	//alert(" TempID: " + tempID +  " MenuDisplayString: " + MenuDisplayStr + " Menu Help String :" + MenuHelpStr + " MenuUrl:" + MenuURLStr + " Target:" + TargetStr);
	addMenu(tempID, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, false, MenuFlagBGColor); 
	LastMSMenu = tempID;
}


function addMenuOpenPopup(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, bICPMenu)
{
	...
	if (!RTL){
		...
		MenuStr += "<span STYLE='text-decoration:none;cursor:hand;font:" + cFont + ";background-color:" + cColor0 + ";color:" + cColor1 + ";' onClick='javascript:window.open(\""+MenuURLStr+"\", \"blank\", \"scrollbars=yes,toolbar=no,width=500,height=400\")' onmouseout="  + char34 + "mouseMenu('out' ,'" + MenuIDStr + "'); hideMenu();" + char34 + 
					" onmouseover=" + char34 + "mouseMenu('over','" + MenuIDStr + "'); doMenu('"+ MenuIDStr + "');" + char34 + ">&nbsp;" + MenuDisplayStr + "&nbsp;</span>";	         
...
//====================================================================	
function setToolbarBGImage(Gif)
{	
	if (Gif != "")
	{
		DefToolbarBGImage = true;
		HTMLStr = HTMLStr.replace("<!--TOOLBAR_BACKGROUND-->", "BACKGROUND-IMAGE: url(" + Gif + ");");		
	}
}

function setMSMenuPaneBGImage(Gif)
{	
	if (Gif != "")
	{
		DefMSMenuPaneBGImage = true;
		HTMLStr = HTMLStr.replace("<!--MSMENUPANE_BACKGROUND-->", "BACKGROUND-IMAGE: url(" + Gif + ");");		
	}
}

function setICPMenuPaneBGImage(Gif)
{	
	if (Gif != "")
	{
		DefICPMenuPaneBGImage = true;
		HTMLStr = HTMLStr.replace("<!--ICPMENUPANE_BACKGROUND-->", "BACKGROUND-IMAGE: url(" + Gif + ");");
	}
}


function setMSMenuCurve(Gif)
{	
	if (Gif != "")
		setBanner(Gif,"","","<!--BEG_MSMENU_CURVE-->","<!--END_MSMENU_CURVE-->");
}

function addMSSubMenuSpace(MenuIDStr)
{	
	tempID = "MS_" + MenuIDStr;
	addSubMenuSpace(tempID,false);
}

function addSubMenuSpace(MenuIDStr, bICPMenu)
{
	var LookUpTag = "<!--" + MenuIDStr + "-->";
	var sPos = HTMLStr.indexOf(LookUpTag);
	if (sPos > 0)
	{
		cColor  = bICPMenu? aDefICPColor[0] : aDefMSColor[0];
		TempStr = newLineChar + "<HR STYLE='color:" + cColor + "' SIZE=1>" + LookUpTag;
		HTMLStr = HTMLStr.replace(LookUpTag, TempStr);
	}
}
function addMenu(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, bICPMenu, bFlagBGColor)
{
	cFont   = bICPMenu? ICPFont : MSFont;
	cColor0 = bICPMenu? aDefICPColor[0] : aDefMSColor[0];
	cColor1 = bICPMenu? aDefICPColor[1] : aDefMSColor[1];
	...
	if (RTL){
		cStyle  = "font:" + cFont + ";background-color:" + cColor0 + ";color:" + cColor1 + ";";
		if (!bFlagBGColor) cStyle  = cStyle.replace(";background-color:" + cColor0, "");
		if (MenuHelpStr == "") MenuHelpStr = MenuDisplayStr; // Shailr. This line should have been before the line MenuStr = newLineChar; but I am tryig to avoid another if..else
			alert (cStyle);
	}
	tagStr  = bICPMenu? "<!--ICP_MENU_TITLES-->" : "<!--MS_MENU_TITLES-->";
	...
	if (!RTL){
		...
		if (MenuURLStr != ""){
			...
			MenuStr += "<font STYLE='text-decoration:none;font:" + cFont + ";background-color:" + cColor0 + ";color:" + cColor1 + ";'>&nbsp;" + MenuDisplayStr + "&nbsp;</font>";
		MenuStr += tagStr;
	}
	if ( RTL ){
			if (bICPMenu) 
	        ...
	        if (MenuURLStr != ""){
		   MenuStr += "<A STYLE='text-decoration:none;font:" + cFont + ";color:" + cColor1 + ";'" +
	             "   TARGET='" + TargetStr + "'" +
			...
			MenuStr += "<font STYLE='text-decoration:none;cursor:hand;font:" + cFont + ";background-color:" + cColor0 + ";color:" + cColor1 + ";'>&nbsp;" + MenuDisplayStr + "&nbsp;</font>";
	...
	if (!bFlagBGColor)
	{
		MenuStr= MenuStr.replace(";background-color:" + cColor0, "");
	}
	HTMLStr = HTMLStr.replace(tagStr, MenuStr);	
	setSubMenuWidth(MenuIDStr,"default",0);
}
